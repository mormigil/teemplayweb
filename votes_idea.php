...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
	if(empty($_SERVER['PATH_INFO'])){
		...
		//votes will need some kind of expiration mechanic so that this query is limited 
		//to no more than roughly 300 votes
		if(!empty($_GET['userid'])&&empty($_GET['ideaid'])){
			$votes = vote_idea::findByUser($_GET['userid'], 0);
			$vote_ids = array();
			foreach($votes as $t){
				$vote_ids[] = $t->getJSON();
			}
		...
		//find all votes from an idea
			...
			$votes = vote_idea::findByIdea($_GET['ideaid'], 0);
			$vote_ids = array();
			foreach($votes as $t){
				$vote_ids[] = $t->getJSON();
			}
		...
		//find the vote of userid on ideaid
			...
			$vote = vote_idea::findByIdeaAndUser($_GET['ideaid'], $_GET['userid']);
			if($vote == null){
				header("HTTP/1.1 400 Bad Request");
	...
	if(empty($_SERVER['PATH_INFO'])){
		...
		$vote = vote_idea::createVote(NULL, $_POST['ideaid'], $_POST['userid']);
		if($vote){
			header("Content-type: application/json");
			print(json_encode($vote->getJSON()));
			exit();
